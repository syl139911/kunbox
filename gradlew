...
#
#   Important for authorized use of the Gradle build container.
#
##############################################################################
...
# Need this for consistency with relative paths from within the project
while [ -h "$app_path" ]; do
    ls=`ls -ld "$app_path"`
    link=`expr "$ls" : '.*-> \(.*\)$'`
...
if [ -n "$JAVA_HOME" ] ; then
    if [ -x "$JAVA_HOME/jre/sh/java" ] ; then
        # IBM's J9 on AIX uses jre/sh/java instead of jre/bin/java
